...
static int parse_MqttRequestBloodPressureMeasurement(uint8_t *msg, int size, uint8_t *out_buffer) {
    ...
    int index = 0;
    // Port-message code
    out_buffer[index+0] = (79 >> 8);
    out_buffer[index+1] = (79 & 0xFF);
    index += 2;
...
static int parse_MqttConfirmBloodPressureMeasurement(uint8_t *msg, int size, uint8_t *out_buffer) {
    ...
    int index = 0;
    // Port-message code
    out_buffer[index+0] = (80 >> 8);
    out_buffer[index+1] = (80 & 0xFF);
    index += 2;
    // Find all forwarded parameters
    int np;
    for (np = 0; np < 0; np++) {
...
void forward_MQTT1_ANDUA651BLEImpl_send_Tellu_TelluCloudBloodPressure(struct ANDUA651BLEImpl_Instance *_instance, time_t observationTime, int16_t health_pressure_diastolic, int16_t health_pressure_mean, int16_t health_pressure_systolic, int16_t health_pulseRate){
    ...
    result = sprintf(&buffer[index], "%.*s", 253-index, "{\"observation\":{");
    if (result >= 0) { index += result; } else { return; }
    // Parameter deviceId
    result = sprintf(&buffer[index], "%.*s", 253-index, "\"deviceId\":");
    if (result >= 0) { index += result; } else { return; }
!!!!!!!!! DEVICE ID !!!!!!!!
    // Parameter observationTime
    result = sprintf(&buffer[index], "%.*s", 253-index, ",\"observationTime\":");
    if (result >= 0) { index += result; } else { return; }
    struct tm tbuf;
    struct tm *timebuf = gmtime(&observationTime);
    result = strftime(&buffer[index], 253-index, "\"%FT%T.000Z\"", timebuf);
    if (result >= 0) { index += result; } else { return; }
    // Parameter observations
    result = sprintf(&buffer[index], "%.*s", 253-index, ",\"observations\":");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 253-index, "[");
    if (result >= 0) { index += result; } else { return; }
    /* ---------- Start Tellu Cloud grouped observations ---------- */
    // Parameter health_pressure_diastolic
    result = sprintf(&buffer[index], "%.*s", 253-index, "{");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 253-index, "\"health.pressure.diastolic\":");
    if (result >= 0) { index += result; } else { return; }
    ...
    result = sprintf(&buffer[index], "%.*s", 253-index, ",{");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 253-index, "\"health.pressure.mean\":");
    if (result >= 0) { index += result; } else { return; }
    ...
    result = sprintf(&buffer[index], "%.*s", 253-index, ",{");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 253-index, "\"health.pressure.systolic\":");
    if (result >= 0) { index += result; } else { return; }
    ...
    result = sprintf(&buffer[index], "%.*s", 253-index, ",{");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 253-index, "\"health.pulseRate\":");
    if (result >= 0) { index += result; } else { return; }
