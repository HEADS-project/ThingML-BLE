...
void forward_MQTT2_ANDUC352BLEImpl_send_Tellu_TelluCloudWeightScale(struct ANDUC352BLEImpl_Instance *_instance, time_t observationTime, float mass){
    ...
    result = sprintf(&buffer[index], "%.*s", 150-index, "{\"observation\":{");
    if (result >= 0) { index += result; } else { return; }
    // Parameter deviceId
    result = sprintf(&buffer[index], "%.*s", 150-index, "\"deviceId\":");
    if (result >= 0) { index += result; } else { return; }
!!!!!!!!! DEVICE ID !!!!!!!!
    // Parameter observationTime
    result = sprintf(&buffer[index], "%.*s", 150-index, ",\"observationTime\":");
    if (result >= 0) { index += result; } else { return; }
    struct tm tbuf;
    struct tm *timebuf = gmtime(&observationTime);
    result = strftime(&buffer[index], 150-index, "\"%FT%T.000Z\"", timebuf);
    if (result >= 0) { index += result; } else { return; }
    // Parameter observations
    result = sprintf(&buffer[index], "%.*s", 150-index, ",\"observations\":");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 150-index, "[");
    if (result >= 0) { index += result; } else { return; }
    /* ---------- Start Tellu Cloud grouped observations ---------- */
    // Parameter mass
    result = sprintf(&buffer[index], "%.*s", 150-index, "{");
    if (result >= 0) { index += result; } else { return; }
    result = sprintf(&buffer[index], "%.*s", 150-index, "\"mass\":");
    if (result >= 0) { index += result; } else { return; }
    if (isnan(mass) || isinf(mass)) {
        result = sprintf(&buffer[index],"%.*s", 150-index, "null");
    } else {
