...
const char *MQTT2_topics[4] = {
    "ThingML",
    "5C:31:3E:5F:00:7A/MqttRequestWeightMeasurement",
    "5C:31:3E:5F:00:7A/MqttConfirmWeightMeasurement",
...
{
    const char *host = "mqtt.tellucloud.com";
    int port = 1883;
    ...
    mosquitto_lib_init();
    MQTT2_mosq = mosquitto_new(client_id, true, _instance);
    if (!MQTT2_mosq) {
...
static int parse_MqttRequestWeightMeasurement(uint8_t *msg, int size, uint8_t *out_buffer) {
    uint8_t *ptr = msg;
    uint8_t *start = NULL;
    uint8_t *end = NULL;
    uint8_t *pstart = NULL;
...
static int parse_MqttConfirmWeightMeasurement(uint8_t *msg, int size, uint8_t *out_buffer) {
    uint8_t *ptr = msg;
    uint8_t *start = NULL;
    uint8_t *end = NULL;
    uint8_t *pstart = NULL;
...
void MQTT2_parser(uint8_t *msg, int size, struct MQTT2_Instance *_instance) {
    ...
    int result = -1;
    if (0) {}
    else if (strncmp("MqttRequestWeightMeasurement", start, end-start) == 0) {
        result = parse_MqttRequestWeightMeasurement(ptr, size-(ptr-msg), enqueue_buffer);
    }
    else if (strncmp("MqttConfirmWeightMeasurement", start, end-start) == 0) {
        result = parse_MqttConfirmWeightMeasurement(ptr, size-(ptr-msg), enqueue_buffer);
    }
