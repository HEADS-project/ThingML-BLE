...
void enqueue_BLEConnecterImpl_Connecter_ConnectTo(struct BLEConnecterImpl_Instance * inst, uint8_t AddressType, bdaddr_t Address, ble_random_number_t LongTermKey, uint16_t EncryptedDiversifier, ble_random_part_t RandomNumber) {
    fifo_lock(&(inst->fifo));
    if ( fifo_byte_available(&(inst->fifo)) > 37 ) {
...
_fifo_enqueue(&(inst->fifo), u_EncryptedDiversifier.bytebuffer[1] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_EncryptedDiversifier.bytebuffer[0] & 0xFF );
...
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[7] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[6] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[5] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[4] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[3] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[2] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[1] & 0xFF );
_fifo_enqueue(&(inst->fifo), u_RandomNumber.bytebuffer[0] & 0xFF );
    }
    fifo_unlock_and_notify(&(inst->fifo));
}
...
void enqueue_BLEConnecterImpl_Connecter_Encrypt(struct BLEConnecterImpl_Instance * inst) {
    ...
    if ( fifo_byte_available(&(inst->fifo)) > 4 ) {
        ...
        _fifo_enqueue(&(inst->fifo), (100 >> 8) & 0xFF );
        _fifo_enqueue(&(inst->fifo), 100 & 0xFF );
...
case 98:{
byte mbuf[37 - 2];
while (mbufi < (37 - 2)) mbuf[mbufi++] = fifo_dequeue(&(_instance->fifo));
fifo_unlock(&(_instance->fifo));
...
switch(portID) {
case 0:{
BLEConnecterImpl_handle_Connecter_ConnectToU(_instance,
...
switch(portID) {
case 0:{
BLEConnecterImpl_handle_Connecter_Encrypt(_instance);
return 1;
break;
}
}
...
switch(portID) {
case 1:{
BLEConnecterImpl_handle_HCIEvents_EncryptionChanged(_instance,
...
switch(portID) {
case 1:{
BLEConnecterImpl_handle_HCIEvents_LEStartEncryptionStatus(_instance,
