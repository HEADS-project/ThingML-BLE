...
// Forwarding of messages timer2::HouseTest::timer::timer_start
void forward_timer2_HouseTest_send_timer_timer_start(struct HouseTest_Instance *_instance, uint8_t id, uint32_t time){
...
uint8_t mbufi_clock_25ms = 2;
dispatch_clock_25ms((mbuf[0] << 8) + mbuf[1] /* instance port*/);
break;
}
case 1:{
byte mbuf[5 - 2];
...
void forward_HouseTest_send_timer_timer_start(struct HouseTest_Instance *_instance, uint8_t id, uint32_t time){
if(_instance->id_timer == test_var.id_timer) {
forward_timer2_HouseTest_send_timer_timer_start(_instance, id, time);
}
}
