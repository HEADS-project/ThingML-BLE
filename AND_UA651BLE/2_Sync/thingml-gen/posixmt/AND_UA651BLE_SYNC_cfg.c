...
enqueue_Main_Gateway_MqttRequestBloodPressureMeasurement(&main_var);
...
if (sender == MQTT_instance.listener_id) {
enqueue_Main_Gateway_MqttConfirmBloodPressureMeasurement(&main_var);
...
void forward_Main_send_Gateway_MqttBloodPressureMeasurement(struct Main_Instance *_instance, float Systolic, float Diastolic, float MeanArterial, uint8_t UnitKPA, uint16_t Year, uint8_t Month, uint8_t Day, uint8_t Hour, uint8_t Minute, uint8_t Second, float Pulse, uint8_t IrregularPulse){
if(_instance->id_Gateway == main_var.id_Gateway) {
forward_MQTT_Main_send_Gateway_MqttBloodPressureMeasurement(_instance, Systolic, Diastolic, MeanArterial, UnitKPA, Year, Month, Day, Hour, Minute, Second, Pulse, IrregularPulse);
}
}
...
register_external_Main_send_Gateway_MqttBloodPressureMeasurement_listener(&forward_Main_send_Gateway_MqttBloodPressureMeasurement);
...
connecterbloodpressure_var.BLEConnecterImpl_ConnectAddress_var = "5C:31:3E:00:4B:6B";
...
connecternotifier_var.BLEConnecterImpl_ConnectAddress_var = "F3:48:7F:62:B8:60";
